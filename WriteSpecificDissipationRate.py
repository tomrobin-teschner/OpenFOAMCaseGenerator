...
def write_boundary_condition(file_manager, boundary_properties, flow_properties, solver_properties):
    ...
    # calculate free-stream specific dissipation rate epsilon
    omega = pow(Parameters.C_MU, -0.25) * pow(k, 0.5) / flow_properties['reference_length']

    # create new boundary file
    file_id = file_manager.create_file('0', 'omega')
    file_manager.write_header(file_id, 'volScalarField', '0', 'omega')
    ...
    for key in boundary_properties:
        ...
        if boundary_properties[key] == Parameters.WALL:
            if solver_properties['wall_modelling'] == Parameters.LOW_RE:
                ...
                BoundaryConditions.omegaWallFunction(file_id, initial_field)
